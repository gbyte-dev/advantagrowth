    ...
    return (
        <div className="relative" ref={containerRef}>
            ...
            >
                ...
                <ChevronDown
                    size={16}
                    className={`ml-2 shrink-0 text-gray-400 transition-transform duration-200 ${open ? "rotate-180 text-violet-600" : ""
                        }`}
                />
            </button>

            {open && (
                <div className="absolute left-0 top-full z-50 mt-1 w-full overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl">
                    ...
                    <ul className="m-0 max-h-56 list-none overflow-y-auto py-1 pl-0 select-none [scrollbar-width:thin] [scrollbar-color:#cbd5e1_#f8fafc] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-slate-50 [&::-webkit-scrollbar-thumb]:bg-slate-300 hover:[&::-webkit-scrollbar-thumb]:bg-slate-400 [&::-webkit-scrollbar-thumb]:rounded-full">
                        {filtered.length === 0 && (
